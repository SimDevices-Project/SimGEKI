/* SPDX-License-Identifier: MPL-2.0
 *
 * The inverse S-box constants are adapted from PN532-Aime-Reader commit
 * 8feaf84860a17f10ebd12c3827a17cc7deaa01c3.
 */
#include "felica-inject.h"

#include "data.h"

#include <string.h>

#define FELICA_INJECT_SPAD_SIZE 16
#define FELICA_INJECT_ACCESS_CODE_OFFSET 6
#define FELICA_INJECT_TABLE_COUNT 8
#define FELICA_INJECT_MIFARE_BLOCK_AIME_ID 2

typedef enum {
  FELICA_INJECT_IDLE,
  FELICA_INJECT_PROBING_FELICA,
  FELICA_INJECT_EMULATING_MIFARE,
} FelicaInjectState;

static FelicaInjectState state;
static uint8_t mifare_uid[4];
static uint8_t mifare_block2[16];

static const uint8_t inverse_sbox[9][256] = {
  {
    
  },
  {
    
  },
  {
    
  },
  {
    
  },
  {
    
  },
  {
    
  },
  {
    
  },
  {
    
  },
  {
    
  },
};

static bool local_read_enabled(void)
{
  return GlobalData != NULL && GlobalData->CardReadMode == 1;
}

static void rotate_right_120(uint8_t data[FELICA_INJECT_SPAD_SIZE])
{
  uint8_t prior = data[14];

  for (uint8_t i = 0; i < 15; i++) {
    uint8_t current = data[i];
    data[i] = (uint8_t)((current >> 5) | ((prior & 0x1F) << 3));
    prior = current;
  }
}

static bool decode_spad0(uint8_t spad0[FELICA_INJECT_SPAD_SIZE])
{
  for (uint8_t i = 0; i < FELICA_INJECT_SPAD_SIZE; i++) {
    spad0[i] = inverse_sbox[8][spad0[i]];
  }

  uint8_t count = (uint8_t)((spad0[15] >> 4) + 7);
  uint16_t table = (uint16_t)(spad0[15] + 5U * count);

  for (uint8_t iteration = 0; iteration < count; iteration++) {
    table -= 5;
    rotate_right_120(spad0);

    const uint8_t *sbox = inverse_sbox[table % FELICA_INJECT_TABLE_COUNT];
    for (uint8_t i = 0; i < 15; i++) {
      spad0[i] = sbox[spad0[i]];
    }
  }

  for (uint8_t i = FELICA_INJECT_ACCESS_CODE_OFFSET; i < FELICA_INJECT_SPAD_SIZE; i++) {
    if ((spad0[i] >> 4) > 9 || (spad0[i] & 0x0F) > 9) {
      return false;
    }
  }
  return true;
}

void FelicaInject_Reset(void)
{
  state = FELICA_INJECT_IDLE;
}

bool FelicaInject_BeginFelicaProbe(const uint8_t idm[8], const uint8_t system_code[2])
{
  FelicaInject_Reset();
  if (!local_read_enabled() || idm == NULL || system_code == NULL ||
      idm[0] != 0x01 || idm[1] != 0x2E ||
      system_code[0] != 0x88 || system_code[1] != 0xB4) {
    return false;
  }

  memcpy(mifare_uid, idm, sizeof(mifare_uid));
  state = FELICA_INJECT_PROBING_FELICA;
  return true;
}

bool FelicaInject_IsProbing(void)
{
  return state == FELICA_INJECT_PROBING_FELICA;
}

void FelicaInject_FinishFelicaProbe(const uint8_t spad0[FELICA_INJECT_SPAD_SIZE])
{
  if (!FelicaInject_IsProbing()) {
    return;
  }

  state = FELICA_INJECT_IDLE;
  if (spad0 == NULL) {
    return;
  }

  uint8_t decoded[FELICA_INJECT_SPAD_SIZE];
  memcpy(decoded, spad0, sizeof(decoded));
  if (decode_spad0(decoded)) {
    memcpy(mifare_block2, decoded, sizeof(mifare_block2));
    state = FELICA_INJECT_EMULATING_MIFARE;
  }
}

uint8_t FelicaInject_FillPollResponse(AIME_Response *res, const uint8_t idm[8], const uint8_t pmm[8])
{
  if (state == FELICA_INJECT_EMULATING_MIFARE && local_read_enabled()) {
    res->count = 1;
    res->type = 0x10;
    res->id_len = sizeof(mifare_uid);
    memcpy(res->mifare_uid, mifare_uid, sizeof(mifare_uid));
    return 7;
  }

  res->count = 1;
  res->type = 0x20;
  res->id_len = 16;
  memcpy(res->IDm, idm, 8);
  memcpy(res->PMm, pmm, 8);
  return 19;
}

bool FelicaInject_EmulateMifareAuth(const uint8_t uid[4])
{
  return state == FELICA_INJECT_EMULATING_MIFARE && local_read_enabled() &&
         uid != NULL && memcmp(uid, mifare_uid, sizeof(mifare_uid)) == 0;
}

bool FelicaInject_EmulateMifareRead(uint8_t block_number, uint8_t block[16])
{
  if (state != FELICA_INJECT_EMULATING_MIFARE || !local_read_enabled() || block == NULL) {
    return false;
  }

  memset(block, 0, 16);
  if (block_number == FELICA_INJECT_MIFARE_BLOCK_AIME_ID) {
    memcpy(block, mifare_block2, sizeof(mifare_block2));
  }
  return true;
}
